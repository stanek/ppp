Below are **complete, drop-in replacements** for *both* files that add a
secondary **blue note** placed with the **right mouse button**.

### How it works

| Action                           | Result                                                                                            |
| -------------------------------- | ------------------------------------------------------------------------------------------------- |
| **Left-click**                   | Toggle a **green** note (class `.on`).                                                            |
| **Right-click**                  | Toggle a **blue** note (class `.blue`).<br>Context-menu is suppressed.                            |
| **Preview sound**                | Plays for **either** colour when a note is turned **on**.                                         |
| **Selection / move / fingering** | Treats green **or** blue as “on” notes. Both colours move together; digits travel with them.      |
| **Persistence**                  | State now stores **green**, **blue**, and **fingering** arrays; everything is restored on reload. |
...
* **Right-click** toggles blue notes and suppresses the browser context-menu.
* **Selection logic** treats blue and green identically.
* **Playback** plays both colours the same sound.
* **Persistence** now stores three arrays: `green`, `blue`, and `fing`.

Enjoy the new dual-colour grid! 🎹
