Below are the **two complete replacements** you need:
...
```css
/* dimmer background for notes that are *outside* the selected key */
.seq td.outkey{
  background:#444;                 /* lighter than default #333 */
}
```

Put it **just above** the existing `.seq td.on` rule so green “on” tiles still override it.

---

## 2 · script.js  (whole file)
...
const SAMPLE_ROOTS = ['A','C','D#','F#'];         // real mp3s
const ROWS = 200;
...
const labelCells=[];                                // refs for labels
...
/* linear column → note with octave */
const NOTES_LINEAR = OCTAVES.flatMap(o=>NOTE_NAMES.map(n=>n+o));

/* pitch-class of every column (0-11) */
const COL_PC = NOTES_LINEAR.map(n=>{
  const base=n.replace(/\d+/,''); return {
...
function loadState(){
  ...
  try{
    ...
    if(s.key)  tonicSel.value=s.key;
    if(s.mode) modeSel.value=s.mode;
    if(s.tempo)tempo.value=s.tempo;
    if(Array.isArray(s.on)) applyOnCells(s.on);
  }catch{}
}
loadState();
updateKeyLabels();            // labels & grid shading after load

/* persist tempo changes */
...
grid.addEventListener('pointerdown',e=>{
  ...
  down=true; dragging=false; sRow=td.parentNode.rowIndex; sCol=td.cellIndex;
  if(sel.size) return;
});
grid.addEventListener('pointerover',e=>{
  ...
  if(!dragging&&(r!==sRow||c!==sCol)){ dragging=true; lasso.style.display='block'; }
  if(dragging){
...
document.addEventListener('keydown',e=>{
  ...
  sel.clear(); moves.forEach(([,d])=>sel.add(d)); saveState();
});
...
### Summary of the new behaviour

1. **Shading:**

   * When you choose a key, every grid column whose pitch-class isn’t in the 7-note scale gains the class `.outkey` → lighter grey (#444).
   * “On” cells override the shade when toggled green.
2. **Persistence:** key, mode, tempo, and grid still save to `localStorage`.
3. **Performance:** shading is done column-wise inside `updateKeyLabels()`; only one pass over the grid per key change.
