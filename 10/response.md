Here’s the **complete, working `script.js`** with the single-cell octave indicators.
Drop this in place of your current file and reload the page.
...
* For each octave we create **one** `<td>` (`ocCell`) whose `colSpan` is 12, so only a single, centered number appears.
* Note-label and key cells are then appended inside the same octave loop, guaranteeing the labels align with the correct octave.

Refresh your page and you should now see “2”, “3”, “4”, “5” each centered between vertical bars over the correct group of 12 keys—with no duplicated numbers.
