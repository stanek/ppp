Below is a **drop-in replacement `script.js`** that **persists**:

* **Key** (tonic + Major/Minor)
* **Tempo**
* **Grid** state (every green “on” cell)

The data is saved to **`localStorage`** after each change and restored automatically on page load.
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Saving**              | `saveState()` serializes `{ key, mode, tempo, on }` and stores it in `localStorage` (`pianoSequencerState`). Runs after every change that affects data.                              |
| **Restoring**           | `loadState()` runs once after grid creation: sets dropdowns, tempo, re-labels piano, and turns on the saved grid cells.                                                              |
| **Grid encoding**       | Each “on” cell is stored as `row × COLS + col` (an integer).                                                                                                                         |
| **Triggers for saving** | • Toggling a single cell<br>• Committing a lasso selection<br>• Arrow-key moves<br>• Changing key or mode (inside `updateKeyLabels`)<br>• Changing tempo (`change` & `input` events) |

Drop this file in, refresh, set up a pattern, reload the page—you’ll see your key, tempo, and grid exactly as you left them. 🎹
