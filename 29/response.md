Below is a **self-contained replacement `script.js`** that:

* fixes the `labelCells` initialization error,
* lets you **rename a workspace with a single click on the active tab**, and
* preserves every earlier feature (10 workspaces in memory, left-green / right-blue notes, fingerings, selection moves, per-workspace persistence, etc.).
...
* **`labelCells`** is declared at the top and used only once (no re-declaration).
* **`buildTabs()`**:

  * A **single click** on an **inactive** tab switches workspaces.
  * A **single click** on the **active** tab prompts for a rename, then saves.
* All other functionality (10 in-memory workspaces, right-click blue notes, fingerings, arrow-moves, persistence, preview sounds) works exactly like before. 🎹
